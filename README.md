...
**问题**：`000001`–`000999` 这个号段在深圳市场是股票，在上海市场是指数，两个市场都存在，**无法自动区分**。

| 号段 | 深圳 SZ (market=0) | 上海 SH (market=1) |
|------|--------------------|---------------------|
| 000001–000999 | 股票（约 900 只 A 股） | 指数（约 800 个指数） |

**示例**：同一个 `000001`，含义完全不同：
...
client.finance("sz000858")  # 五粮液

# 上海指数
client.quote("sh000001")  # 上证指数
client.kline("sh000016")  # 上证50

# 不推荐 — 直接抛 ValueError
client.quote("000001")  # 歧义！需加 sz/sh 前缀
```

**报错信息**：不带前缀调用时，`split_code` 会抛出 `ValueError: ambiguous code '000001': 00-prefix codes exist in both SZ (stocks) and SH (indices). Use 'sz000001' or 'sh000001' to disambiguate.`，按提示加前缀即可。

**快速判断归属**：需要确认某只 00 开头代码属于哪个市场时，可查代码表：

```python
# 查深圳代码表
sz = [c for c in client.codes_all(0) if c["code"] == "000001"]
# 查上海代码表
sh = [c for c in client.codes_all(1) if c["code"] == "000001"]
```
