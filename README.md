...
通达信全协议解析器 — 从零构建的纯 Python 二进制协议实现，零外部依赖。

支持 7709 (A股/ETF) 和 7727 (期货/期权) 双端口，覆盖 **31 个命令**，内置**主站并发扫描器**和**本地计算引擎**。

## 缘起

本项目诞生于对 [eltdx](https://pypi.org/project/eltdx/) 源码的深入阅读——在理解其帧格式、varint 编解码、命令号体系之后，决定不直接封装调用，而是从 TCP Socket 层面重新实现整个协议栈。过程中大量参考了以下社区先驱的逆向成果：

- **[pytdx](https://github.com/rainx/pytdx)** — 最早的 Python 通达信协议实现，由 rainx 维护，为后来者铺平了道路
- **[eltdx](https://github.com/nicolossus/eltdx)** — 现代简洁的实现，提供了 7709 主站列表、多线程探测、分页拉取等工程范式，本项目的扫描器设计深受其启发
- **通达信客户端逆向社区** — 无数匿名贡献者在 CSDN、知乎、掘金上分享的命令号对照表、协议帧布局、期货 80 字节握手机制，构成了本项目期货模块的理论基础

站在这些先行者的肩膀上，本项目做了三件不同的事：

1. **双端口统一** — 7709 (股票) 与 7727 (期货) 共用同一套 Tube 传输管道、Frame 编解码、数据模型，消除重复逻辑
2. **本地计算引擎** — 将复权因子、换手率、除权除息解析、09:25 竞价快照从服务端依赖中解放，在客户端本地完成
3. **零依赖** — 只用 Python 标准库的 `socket` + `struct` + `zlib` + `threading`，不引入任何第三方包

感谢以上项目和社区，没有他们的逆向工作就不会有这个项目。
